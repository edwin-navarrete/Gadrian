...
public class Mood : IEquatable<Mood>{
	...
	private const float PERPLEX_THRESHOLD = 0.08f;

	// The Basic Moods. Are Immutable by design.
	public readonly static Mood INDIFERENT = new Mood(Vector2.zero);
	public readonly static Mood SAD = new Mood( Vector3.up * 3e-5f );
	...
	public Mood(Vector3 value){
		this.value = value;
	}

	public Mood(Vector3 value, bool accum){
		this.value = value;
		this.accum = accum;
	}
	...
	public static Mood operator +(Mood m1, Mood m2) 
	{
		if(m1.Equals(Mood.SAD) || m2.Equals(Mood.SAD)){
			Debug.LogWarning("Backfall to SAD for:"+m1+","+m2);
	...
	public Feeling getFeel(){
		...
		float orientation = Vector3.Angle ( Vector2.up, value );
		if( value.x < 0)
	...
	{
		return string.Format ( "[Feel:{0}, intensity:{1}, value:{2}]", getFeel(), getIntensity(), value );
	}
}
